...
public abstract class AbstractScmCoordinator implements ScmCoordinator {
    ...
    public AbstractScmCoordinator(Repository repository, BuildLogger buildLogger) {
     ...
     *
     * @param repository    The repository that is the actual SCM repository that will perform the operations.
     * @param configuration The build's configuration.
     * @param planKey       The plan key of the plan that this build is a part of.
     * @return SCM coordinator according to the repository type.
     */
    public static ScmCoordinator createScmCoordinator(Repository repository, Map<String, String> configuration,
            PlanKey planKey, BuildLogger buildLogger) {
        if (repository instanceof SvnRepository) {
            return new SubversionCoordinator(repository, configuration, planKey, buildLogger);
        }
        // Git is optional SCM so we cannot use the class here
        if (isGitScm(repository)) {
            return new GitCoordinator(repository, configuration, planKey, buildLogger);
        }
