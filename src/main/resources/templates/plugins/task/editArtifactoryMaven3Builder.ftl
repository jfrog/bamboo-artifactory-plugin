...
<script>
    ...
    function displayMaven3ArtifactoryConfigs(serverId) {
        let configDiv = document.getElementById('maven3ArtifactoryConfigDiv');
        ...
        if ((serverId == null) || (serverId.length === 0) || (-1 === serverId)) {
            ...
            let urlSelect = document.getElementsByName('builder.artifactoryMaven3Builder.artifactoryServerId')[0];
    ...
    function displayResolutionMaven3ArtifactoryConfigs(serverId) {
        let configDiv = document.getElementById('maven3ArtifactoryResolutionConfigDiv');
        let credentialsUserName = configDiv.getElementsByTagName('input')[2].value;
        let credentialsPassword = configDiv.getElementsByTagName('input')[3].value;
    ...
    function loadMaven3ResolvingRepoKeys(serverId, credentialsUserName, credentialsPassword) {
        AJS.$.ajax({
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                let errorMessage = 'An error has occurred while retrieving the resolution repository list.<br>' +
                    'Response: ' + XMLHttpRequest.status + ', ' + XMLHttpRequest.statusText + '.<br>';
                if (XMLHttpRequest.status === 404) {
                    errorMessage +=
                        'Please make sure that the Artifactory Server Configuration Management Servlet is accessible.'
    ...
    let deployErrorDiv = document.getElementById('deploy-repo-error');
    let deployRepoSelect = document.getElementsByName('builder.artifactoryMaven3Builder.deployableRepo')[0];
    let resolveRepoSelect = document.getElementsByName('builder.artifactoryMaven3Builder.resolutionRepo')[0];
    // Init error-divs.
    deployErrorDiv.innerHTML = '';
    deployErrorDiv.style.display = 'none';
    resolveErrorDiv.innerHTML = '';
